...
Object.keys(prisma._dmmf.modelMap).map(name => v9_models.push(name.toLowerCase()));
...
app.get('/api', async (req, res) => {
  return res.json({ timestamp: Date.now(), models: v9_models, coreCore: 'Vc9_initPrisma', version: '1.0.0' });
});

app.get('/api/:type', async (req, res) => {
  return res.json((v9_models.indexOf(req.params.type) > -1) ? await prisma[req.params.type].findMany({ take: 5 }) : {});
});


app.get('/api/page/:slug', async (req, res) => {
  return res.json(await $_data('page', req.params.slug));
});

app.get('/api/post/:slug', async (req, res) => {
  return res.json(await $_data('post', req.params.slug));
});
