...
const api = {
  ...
  typeBySlug: async (req, res) => res.end(await v9_stringify(await model_exists(req.params.type) ? await prisma[req.params.type].findUnique({ where: { slug: req.params.slug } }) : {}), 'utf-8'),
  userByUsername: async (req, res) => res.end(await v9_stringify(await prisma.user.findUnique({ where: { username: req.params.username } })), 'utf-8'),
};
